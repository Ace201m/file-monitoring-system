...
class Data:
    def __init__(self, data_id=-1, event=None):
        if data_id == -1 or data_id == 0:
            ...
            self.data[Names.DB_DATA_COLLECTION_SIGN] = file_hash.getSignature(
            ) if os.path.exists(filepath) else 'EMPTY'

            self.data[Names.DB_DATA_COLLECTION_CONFIG] = prompt(
            ) if data_id == -1 else 'EMPTY'
            self.data[Names.DB_DATA_COLLECTION_TYPE] = 'DIR' if event.is_directory else 'FILE'
            self.data[Names.DB_DATA_COLLECTION_OWNER] = str(
                fileInfo.owner()) if data_id == -1 else 'EMPTY'
            self.data[Names.DB_DATA_COLLECTION_PARENT_OWNER] = str(
                fileInfo.parent.owner())
        else:
            database = Database()
